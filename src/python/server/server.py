...
def getFileAsByteArray(filename):
    fo = open(os.path.join("./source",filename), "rb")
    return fo.read()
...
class Connection:
    ...
    def recv(self):
        if self.state != RECV:
            return
        data = self.socket.recv(1025)
        if not data or data[0] == INSTRUCTION_CLOSE:
            self.socket.close()
            return True
        self.data = data[1:].decode('ascii')
        if data[0] == INSTRUCTION_REQUEST_FILE or data[0] == INSTRUCTION_HASH:
...
def precomputeHashes():
    global hashes
    hashes = dict();
    relativePathToAllFiles = [os.path.relpath(os.path.join(r, file), "./source") for r, s, f in os.walk("./source") for file in f]
    for path in relativePathToAllFiles:
        print("Computing hash of: ", path)
        hashes[path] = getHashOfFile(path)
    print("Hashes precomputed.")
